...
import com.rlis.system.mapper.RlSysDictTypeMapper;
...
/**
 * @ClassName: RlSysDictTypeServiceImpl
 * @Description: 字典 业务层处理
 * @Author tangxiaohui
 * @Copyright: Copyright (c) 2020
 * @Company: 成都信通网易医疗科技发展有限公司
 * @DateTime 2020/7/10 13:19
 */
@Service
public class RlSysDictTypeServiceImpl implements IRlSysDictTypeService
{
    @Autowired
    private RlSysDictTypeMapper dictTypeMapper;
    ...
    public void init()
    {
        List<RlSysDictType> dictTypeList = dictTypeMapper.selectDictTypeAll();
        for (RlSysDictType dictType : dictTypeList)
        {
            List<RlSysDictData> dictDatas = dictDataMapper.selectDictDataByType(dictType.getDictType());
    ...
    {
        List<RlSysDictData> dictDatas = DictUtils.getDictCache(dictType);
    ...
    public int deleteDictTypeByIds(String ids)
    {
        ...
        {
            RlSysDictType dictType = selectDictTypeById(dictId);
    ...
    {
        RlSysDictType oldDict = dictTypeMapper.selectDictTypeById(dictType.getDictId());
     ...
     */
    @Override
    public String checkDictTypeUnique(RlSysDictType dict)
    {
        Long dictId = StringUtils.isNull(dict.getDictId()) ? -1L : dict.getDictId();
        RlSysDictType dictType = dictTypeMapper.checkDictTypeUnique(dict.getDictType());
    ...
    public List<Ztree> selectDictTree(RlSysDictType dictType)
    {
        ...
        List<RlSysDictType> dictList = dictTypeMapper.selectDictTypeList(dictType);
        for (RlSysDictType dict : dictList)
        {
    ...
    public String transDictName(RlSysDictType dictType)
    {
